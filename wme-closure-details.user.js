...
function observeContentsPane() {
  var observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      ...
      var selectedIDs = Waze.selectionManager.selectedItems.filter(function(obj) {
      ...
      for (var i = 0; i < closureBlocks.length; i++) {
        ...
        if (matchedClosure) {
          ...
          info.title = (matchedClosure.permanent ? 'Ignores traffic' : 'Listens to traffic') + '<br/>Created by: ' + getUsername(matchedClosure.createdBy) + '<br/>Created on: ' + I18n.l('time.formats.long', matchedClosure.createdOn);
          if (matchedClosure.updatedBy) {
            info.title += '<br/>Updated by: ' + getUsername(matchedClosure.updatedBy);
          }
          if (matchedClosure.updatedOn) {
            info.title += '<br/>Updated on: ' + I18n.l('time.formats.long', matchedClosure.updatedOn);
          }
          ...
          if (matchedClosure.reason != '') {
            description.appendChild(document.createTextNode(matchedClosure.reason));
          } else {

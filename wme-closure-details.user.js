  ...
  var observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      ...
      for (var i = 0; i < closureBlocks.length; i++) {
        ...
        var startDate = getDateText(closureBlock, '.start-date');
        var endDate = getDateText(closureBlock, '.end-date');
        ...
        if (match) {
          ...
          info.title = (match.permanent ? 'WME-entered closure' : 'In-app closure') + '<br/>Created by: ' + getUsername(match.createdBy) + '<br/>Created on: ' + I18n.l('time.formats.long', match.createdOn);
          if (match.updatedBy) {
...
function getDateText(origin, selector) {
  return origin.querySelector(selector + ' .date').textContent + ' ' + origin.querySelector(selector + ' .time').textContent;
}
